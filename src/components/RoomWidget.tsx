...
export const RoomWidget: React.FC<RoomWidgetProps> = ({ rooms, onDeviceToggle }) => {
        ...
        <div className="mt-4 pt-4 border-t border-secondary-100 flex items-center justify-between text-sm">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-secondary-500">
              <span>🌡️</span>
              <span>{currentRoom.temperature?.toFixed(1)}°C</span>
            </div>
            <div className="flex items-center gap-2 text-secondary-500">
              <span>💧</span>
...
const DeviceCardCompact: React.FC<{
  device: Device;
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative min-w-[180px] max-w-[180px] bg-white rounded-2xl p-4 shadow-sm border border-secondary-100 cursor-pointer transition-all duration-300",
        device.isOn && "shadow-md"
      )}
    >
      {/* Active Indicator */}
      {device.isOn && (
